"""
Apartments.ng adapter.

RECONNAISSANCE, 2026-09-24:
  - robots.txt permits listing paths, disallows only /oc-admin/
  - Listing portal with editorial blog
  - robots.txt: https://apartments.ng/robots.txt
  - Sitemaps: not detected in robots.txt

DISCOVERY STRATEGY
------------------
Since no sitemap is published in robots.txt, discovery falls back to category
pagination. The site structure for short-lets appears to be under paths like
/short-let/<state>/<area>/ or similar. This will need refinement once live
pages are inspected.
"""
...
class ApartmentsNgAdapter:
    """Apartments.ng discovery adapter."""

    name = "apartments_ng"
    layer = SourceLayer.DISCOVERY
    host = "apartments.ng"
    base_url = "https://apartments.ng"
    sitemap_index = "https://apartments.ng/sitemap.xml"
    ...
    def list_page_urls(self, state_code: str, area: Optional[str] = None) -> Iterator[str]:
        ...
        if not slug:
            raise ValueError(f"Apartments.ng has no slug for state '{state_code}'")
