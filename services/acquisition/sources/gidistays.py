...
RECONNAISSANCE, 2026-09-24:
  - robots.txt: no disallow rules, has sitemap.xml
  - An operator's own site marketing its own apartments in Lagos
  - Layer: OPERATOR_SITE (not a portal, single operator)
  - robots.txt: https://gidistays.com/robots.txt
  - Sitemap: https://gidistays.com/sitemap.xml

DISCOVERY STRATEGY
------------------
This is an operator's own site, not a multi-operator portal. Discovery uses
their published sitemap to find all property pages. The operator is the same
across all listings.
"""
...
class GidiStaysAdapter:
    """GidiStays operator site adapter."""

    name = "gidistays"
    layer = SourceLayer.DISCOVERY
    host = "gidistays.com"
    base_url = "https://gidistays.com"
    sitemap_index = "https://gidistays.com/sitemap.xml"

    def sitemap_listing_urls(self, transport) -> list[str]:
        """Fetch all URLs from the published sitemap."""
        try:
    ...
    def discover(self, transport, state_code: str, area: Optional[str] = None) -> Iterator[str]:
        """Yield candidate listing URLs from sitemap.

        For operator sites, state/area filtering is less relevant since it's
        one operator's portfolio. We yield all property-like URLs from the sitemap.
        """
        listing_urls = self.sitemap_listing_urls(transport)

        # Filter for property-like paths (exclude blog, about, contact, etc.)
        property_paths = [
        ...
        # Fallback: yield all non-admin URLs from sitemap
        for url in listing_urls:
            if not any(skip in url.lower() for skip in ["/admin/", "/wp-admin/", "/blog/", "/about/", "/contact/"]):
                yield url
    ...
    def _is_listing_url(self, url: str) -> bool:
        """Accept any property-like URL on this operator's site."""
    ...
    def _location_from_url(self, url: str) -> dict[str, Optional[str]]:
        """Operator site: default to Lagos unless specified in URL."""
        return {"state": "LA", "city": "Lagos", "area": None}


__all__ = ["GidiStaysAdapter"]